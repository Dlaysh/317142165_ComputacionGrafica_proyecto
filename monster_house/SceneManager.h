...
#include <functional>
glm::vec3 rotateAroundX(const glm::vec3& vec, float angleDegrees) {
    ...
    void update(float deltaTime) {
        ...
        for (auto& pair : satelliteLights) {
            if (pair.satellite != nullptr) {
                glm::vec3 leadingPos = pair.satellite->getLeadingOrbitPosition(0.1f);
				// rotar alrededor del eje Y para simular inclinación de la luz
